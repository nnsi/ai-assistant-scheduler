...
export const CalendarProvider = ({
  ...
  const getStoredSelectedIds = (): string[] => {
    const stored = storage.getSync(STORAGE_KEY_SELECTED);
    if (stored) {
      try {
  ...
  const getStoredKnownIds = (): string[] => {
    const stored = storage.getSync(STORAGE_KEY_KNOWN);
    if (stored) {
      try {
        return JSON.parse(stored);
      } catch {
  ...
  const getStoredDefaultId = (): string | null => {
    return storage.getSync(STORAGE_KEY_DEFAULT);
  };
  ...
  useEffect(() => {
    ...
    if (validSelectedIds.length === 0) {
      ...
      storage.setSync(STORAGE_KEY_SELECTED, JSON.stringify(validIds));
      ...
      storage.setSync(STORAGE_KEY_SELECTED, JSON.stringify(newSelectedIds));
    } else if (validSelectedIds.length !== selectedCalendarIds.length) {
      // 削除されたカレンダーがある場合のみ更新
      setSelectedCalendarIds(validSelectedIds);
      storage.setSync(STORAGE_KEY_SELECTED, JSON.stringify(validSelectedIds));
    }

    // 既知のカレンダーIDを更新してローカルストレージに保存
    knownCalendarIdsRef.current = new Set(validIds);
    storage.setSync(STORAGE_KEY_KNOWN, JSON.stringify(validIds));
    ...
    if (!defaultCalendarId || !validIds.includes(defaultCalendarId)) {
      ...
      if (newDefault) {
        ...
        storage.setSync(STORAGE_KEY_DEFAULT, newDefault);
      }
    }
  }, [calendars, selectedCalendarIds, defaultCalendarId, storage]);
  ...
  const selectAllCalendars = () => {
    ...
    storage.setSync(STORAGE_KEY_SELECTED, JSON.stringify(allIds));
  };

  const selectOnlyCalendar = (id: string) => {
    setSelectedCalendarIds([id]);
    storage.setSync(STORAGE_KEY_SELECTED, JSON.stringify([id]));
  };

  const setDefaultCalendar = (id: string) => {
    setDefaultCalendarIdState(id);
    storage.setSync(STORAGE_KEY_DEFAULT, id);
  };
