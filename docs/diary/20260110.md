...
## リファクタリング Phase 2 実装（夜のセッション）

### やったこと

ユーザーから「6つのタスクを実装して」という依頼を受けた。
...
## コード生成ツールとスキル追加（深夜のセッション）

### やったこと

ユーザーとの議論から、新機能開発時のボイラープレート削減のためコード生成ツールを作成した。

#### 1. plopによるfeature生成ツール
...
- `infra/drizzle/<name>Repo.ts` - Drizzle Implementation（toEntity/toRowはTODO）
- `feature/<name>/route.ts` - Hono CRUD Route
- `feature/<name>/usecase/*.ts` - Create, GetAll, Update, Delete

kebab-caseの名前（例: `task-label`）も正しく`taskLabelRoute`に変換される。

#### 2. `/new-api` スキル

API新規開発時のワークフローとチェックリストをまとめたスキルを追加。

- 設計フェーズ（エンティティ設計、API仕様の明確化）
- コード生成
- スキーマ定義チェックリスト（shared/backend/test全ての同期）
- 実装のポイント（日時、型安全性、認証、E2Eモック）
- テスト・完了確認

### 議論の経緯

最初はRepositoryを生成対象に含めるか迷ったが、ユーザーと議論して以下の結論に至った：

**生成すべき**:
- Repository Interface（パターンが明確）
- Drizzle Implementation（骨格）
- Route（CRUD endpoints）
- Usecase（Create, GetAll, Update, Delete）

**生成しない**:
- Domain Model（ビジネスロジックが固有すぎる）
- Frontend（パターンがまだ流動的）
- Test（テスト内容がfeature依存）

### 設計判断

plopをbackendパッケージではなくルートに配置した。理由：
- 将来フロントエンドでも使う可能性がある
- monorepo全体で一元管理したい

テンプレートは `plop-templates/backend/` に配置し、将来 `plop-templates/frontend/` を追加できる構造にした。

### トークン削減効果の期待

従来：「categoryを参考に新しいfeature作って」→ AIがcategory読んで、構造理解して、生成
改善後：`pnpm generate feature -- --name xxx` → 基本構造完成 → AIは差分だけ実装

これによりAI生成の揺らぎ（import順序のブレ、Result型の使い忘れ等）も抑制できる。

### 感想

ユーザーが「コード生成ツール作っても良いと思う」と提案してきた時点で、すでにボイラープレートの問題は認識していたようだ。議論を通じて「何を生成すべきか」「何を生成しないか」の境界を明確にできた。

特にDomain Modelを生成しない判断は重要。ファクトリ関数やバリデーションロジックはfeature固有なので、テンプレート化すると逆に邪魔になる。「テンプレート化できる」と「テンプレート化すべき」は違う。

スキルとして `/new-api` を追加したことで、将来のAIセッションでも一貫したワークフローが使える。CLAUDE.mdに書くより、スキルとして呼び出せる方が便利だし、チェックリストを毎回参照できる。

### 将来のClaude Codeへ

新しいfeatureを追加するときは：

1. まず `pnpm generate feature -- --name xxx` を実行
2. 生成後のメッセージに従って必要なファイルを作成
3. `/new-api` スキルを参照してチェックリストを確認

テンプレートの場所: `plop-templates/backend/`
